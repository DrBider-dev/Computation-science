...
public class BinarySearch extends javax.swing.JFrame {
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(BinarySearch.class.getName());
    ...
    private void initComponents() {
        ...
        logo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Assets/BinarySearchTitle.png"))); // NOI18N
    ...
    private void guardarArray(Integer[] array) {
        ...
        fileChooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("Archivos de Búsqueda Binaria", "bin"));
    ...
    private Integer[] abrirArray() {
        ...
        if (option == JFileChooser.APPROVE_OPTION) {
            ...
            if (!file.getName().toLowerCase().endsWith(".bin")) {
                JOptionPane.showMessageDialog(this, "El archivo seleccionado no corresponde a una búsqueda binaria.",
    ...
    private void onInsert() {
        try {
            ...
            // 3. buscar primer espacio libre (inserción lineal)
            int freeIndex = -1;
            ...
            // 4. ordenar el array dejando nulls al final
    ...
    private void onSearch() {
        try {
            ...
            while (low <= high) {
                int mid = (low + high) / 2; // siempre redondea hacia la izquierda
                steps.add(mid);

                if (array[mid] == null) {
                    // si es null, no podemos comparar, pero seguimos buscando
                    // para mantener la animación intacta
                    // asumimos que seguimos el algoritmo estándar y reducimos el rango
                    // para evitar bucles infinitos, podemos mover el high hacia la izquierda
                    high = mid - 1;
                    continue;
                }
    ...
    private void onDelete() {
        try {
            ...
            // 2. búsqueda binaria - preparar los pasos para la animación
            refreshCellsUI();
            clearHighlights();
            ...
            // Simular la búsqueda binaria para obtener los pasos
            int low = 0;
            int high = array.length - 1;

            // ignorar los nulls al final
//            while (high >= 0 && array[high] == null) {
//                high--;
//            }

            while (low <= high) {
                int mid = (low + high) / 2;
                steps.add(mid); // Añadir el paso actual a la animación
                System.out.println(mid + "posicion agregada");
            ...
            // 3. Animar la búsqueda binaria y eliminar con callback
            animateDelete(steps, foundIndex[0], () -> {
                if (foundIndex[0] != -1) {
                    array[foundIndex[0]] = null;

                    // ordenar dejando nulls al final
                    Arrays.sort(array, (a, b) -> {
                        if (a == null && b == null) return 0;
                        if (a == null) return 1;  // nulls al final
                        if (b == null) return -1;
                    ...
                    JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(panelCells), 
                        "Clave " + value + " eliminada correctamente",
                        "Eliminado",
                        JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(panelCells), 
    ...
    private Component getCellComponent(int index) {
        int compIndex = index + 1; // margen al principio
